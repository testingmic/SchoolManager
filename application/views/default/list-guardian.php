...
foreach($guardian_list as $kkey => $each) {
    ...
    if($hasUpdate) {
        $action .= "&nbsp;<a href='{$baseUrl}update-guardian/{$each->user_id}/update' class='btn btn-sm btn-outline-success'><i class='fa fa-edit'></i></a>";
    }
    if($hasDelete) {
        $action .= "&nbsp;<a href='#' onclick='return delete_record(\"{$each->user_id}\", \"guardian\");' class='btn btn-sm btn-outline-danger'><i class='fa fa-trash'></i></a>";
    }
