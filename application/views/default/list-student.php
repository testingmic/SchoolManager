...
foreach($student_list["data"] as $key => $each) {
    ...
    if($hasUpdate) {
        $action .= "&nbsp;<a title='Click to update student record' href='#' onclick='return loadPage(\"{$baseUrl}update-student/{$each->user_id}/update\");' class='btn mb-1 btn-sm btn-outline-success'><i class='fa fa-edit'></i></a>";
    }
    if($hasDelete) {
