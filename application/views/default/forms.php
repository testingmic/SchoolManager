...
class Forms extends Myschoolgh {
    ...
    private function assignment_template(stdClass $params, $disabled = null) {
        ...
        // class filter
        $classFilter = $isTutor ? "AND id IN ".$this->inList($defaultUser->class_ids) : null;

        // get the classes list
        $classes_list = $this->pushQuery("name, id, item_id, payment_module", "classes", "client_id='{$params->clientId}' AND status='1' {$classFilter}");
                ...
                <div class='row'>
                    ...
                    <div class=\"".(isset($params->class) ? $params->class : "col-md-12")." text-left\">
                        ...
                        if(!isset($params->no_title)) {
                            $html_content .= "<label>Attachments <small class='text-danger'>(Max size <strong>{$this->max_attachment_size}MB</strong>)</small></label><br>";
                        }
                    $html_content .= "
                            <div class=\"ml-3\">
                                <button type='button' id='ajax-upload-input' class='btn btn-outline-primary'>Select Files</button>
                                <input hidden ".(isset($params->accept) && !empty($params->accept) ? "accept='{$params->accept}'" : null)." ".(isset($params->ismultiple) && !empty($params->ismultiple) ? "multiple" : null)." class='form-control cursor attachment_file_upload' data-form_item_id=\"".($params->item_id ?? "temp_attachment")."\" data-form_module=\"".($params->module ?? null)."\" type=\"file\" name=\"attachment_file_upload\" id=\"attachment_file_upload\">
                            </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-lg-4 col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" value="'.($userData->unique_id ?? null).'" '.($isData ? "disabled='disabled'" : null).' name="unique_id" id="unique_id" class="form-control text-uppercase">
                            </div>
                        </div>
                        <div class="col-lg-4 col-md-6">
                            <div class="form-group">
            ...
            <div class="row mb-4 border-bottom pb-3">
                ...
                <div class="col-lg-4 col-md-6">
                    <div class="form-group">
                        ...
                        <select '.$isAdmin.' '.(!empty($itemData->name) && $isData ? null : "multiple").' data-width="100%" '.($isData ? 'name="class_id" id="class_id"' : 'name="class_id[]" id="class_id[]"').' class="form-control selectpicker">
        ...
        <form class="ajax-data-form" autocomplete="Off" id="ajax-data-form-content" enctype="multipart/form-data" action="'.$baseUrl.'api/users/'.( $isData ? "update" : "add").'" method="POST">
            <div class="row mb-3">
                ...
                <div class="col-lg-4 col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" data-maxdate="'.date("Y-m-d", strtotime("+1 month")).'" value="'.($userData->enrollment_date ?? null).'" name="enrollment_date" id="enrollment_date" class="form-control datepicker">
                    </div>
                </div>
                <div class="col-lg-4 col-md-6">
                    <div class="form-group">
            ...
            <div class="row mb-3">
                ...
                <div class="col-lg-12 col-md-6 '.($isData && $userData->user_type !== "teacher" ? "hidden" : "").'" data-value="course_ids_container">
                    <div class="form-group">
                        <label for="courses_ids">Subjects <span class="text-danger">(Select subjects taught by this teacher)</span></label>
                ...
                <div class="col-lg-12 col-md-6 '.($isData && $userData->user_type !== "teacher" ? "hidden" : "").'" data-value="course_ids_container">
                    <div class="form-group">
                        <label for="class_id">Class <span class="text-danger">(Select the classes this teacher teaches/supervises)</span></label>
                        <select multiple data-width="100%" name="class_id[]" id="class_id" class="form-control selectpicker">
                            <option value="">Select Class</option>';
                            foreach($this->pushQuery("id, name", "classes", "status='1' AND client_id='{$clientId}'") as $each) {
                                $response .= "<option ".($isData && in_array($each->id, $userData->class_ids) ? "selected" : null)." value=\"{$each->id}\">{$each->name}</option>";                            
                            }
                        $response .= '</select>
                    </div>
                </div>
    ...
    public function settings_form($clientId, $form_id = "ajax-data-form-content") {
        ...
        <div class="row">
            ...
            <div class="col-lg-9 pt-3 col-md-12 border-top">
                <div class="form-group text-right">
    ...
    public function bank_accounts_form(stdClass $params) {
        ...
        $html .= "<div class=\"row\">";
        $html .= "<div class=\"col-md-6\" align=\"left\">";
        $html .= "<button class=\"btn btn-outline-danger btn-sm\" onclick=\"return reset_account_form('api/accounting/add_account', 'Create Account')\" type=\"button\">Cancel</button>";
        $html .= "</div>";
        $html .= "<div class=\"col-md-6\" align=\"right\">";
        $html .= "<button class=\"btn btn-outline-success btn-sm\" data-function=\"save\" type=\"button-submit\"><i class=\"fa fa-save\"></i> Save</button>";
        $html .= "</div>";
        $html .= "</div>";
        $html .= "<form>";
        $html .= "</div>";
    ...
    public function bus_form($data = null) {
        ...
        $html = '<div data-backdrop="static" data-keyboard="false" class="modal fade" id="busModal">
            <form autocomplete="Off" action="'.$this->baseUrl.'api/buses/save" class="ajax-data-form" id="ajax-data-form-content">
