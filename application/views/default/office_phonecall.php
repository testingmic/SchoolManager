...
if(!$accessObject->hasAccess("view", "phone_call_log")) {
    // parse error message
    ...
    if(!$singleData) {
        ...
        foreach($results_array as $key => $each) {
            ...
            $action = "<button title='View Record Details' onclick='return load(\"office_phonecall/{$each->item_id}\");' class='btn btn-sm mb-1 btn-outline-primary'><i class='fa fa-eye'></i></button>";
            
            $results_list .= "<tr data-row_id=\"{$each->id}\">";
            $results_list .= "<td>".($key+1)."</td>";
            $results_list .= "<td><span class='user_name' onclick='return load(\"office_phonecall/{$each->item_id}\");'>{$each->content->fullname}</span></td>";
        ...
        // set the form
        $the_form = load_class("forms", "controllers")->phonecall_log_form();
    }
        ...
        <section class="section">
                ...
                <div class="section-header-breadcrumb">
                    ...
                    '.($singleData || !empty($data) ? 
                        '<div class="breadcrumb-item">
                            <a href="'.$baseUrl.'office_phonecall">Phone Call Logs</a>
                        </div>' : null
                    ).'
            ...
            <div class="row">
                ...
                '.(!$singleData ?
                    ...
                    <div class="card">
                        <div class="card-header">Add Phone Call Log</div>
                        <div class="card-body">
                            '.$the_form.'
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-sm-12 col-md-8">
                        <div class="card">
                            <div class="card-header">Phone Call Log List</div>
                            <div class="card-body">
