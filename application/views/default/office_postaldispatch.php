...
if(!$accessObject->hasAccess("view", "postal_dispatch")) {
    // parse error message
    ...
    if(!$singleData) {
        ...
        foreach($results_array as $key => $each) {
            ...
            $action = "<button title='View Record Details' onclick='return load(\"office_postaldispatch/{$each->item_id}\");' class='btn btn-sm mb-1 btn-outline-primary'><i class='fa fa-eye'></i></button>";
            
            $results_list .= "<tr data-row_id=\"{$each->id}\">";
            $results_list .= "<td>".($key+1)."</td>";
            $results_list .= "<td><span class='user_name' onclick='return load(\"office_postaldispatch/{$each->item_id}\");'>{$each->content->to}</span></td>";
            $results_list .= "<td>{$each->content->reference}</td>";
            $results_list .= "<td>{$each->content->from}</td>";
        ...
        // set the form
        $the_form = load_class("forms", "controllers")->postal_form('dispatch');
        ...
        <section class="section">
                ...
                <div class="section-header-breadcrumb">
                    ...
                    '.($singleData || !empty($data) ? 
                        '<div class="breadcrumb-item">
                            <a href="'.$baseUrl.'office_postaldispatch">Postal Dispatch List</a>
                        </div>' : null
                    ).'
            ...
            <div class="row">
                ...
                '.(!$singleData ?
                    ...
                    <div class="card">
                        <div class="card-header">Add Postal Dispatch</div>
                        <div class="card-body">
                            '.$the_form.'
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-sm-12 col-md-8">
                        <div class="card">
                            <div class="card-header">Postal Dispatch List</div>
                            <div class="card-body">
