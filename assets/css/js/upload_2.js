var the_file,
    file_input = $(`input[class~="_attachment_file_upload"]`),
    dd_container = $(`form[class~='ajax-data-form'] div[class~="post-attachment"]`);

$(`input[class~="_attachment_file_upload"]`).change(function() {
    let file_input = $(this);
    ...
    $.each(files_list, async function(i, the_file) {
        ...
        fd.append('_attachment_file_upload', the_file);
        ...
        fd.delete("_attachment_file_upload");
    });

});

$(`button[id='_ajax-upload-input']`).on("click", function() {
    $(`input[class~="_attachment_file_upload"]`).trigger("click");
});
...
async function ajax_file_upload(formdata) {
    ...
    $(`input[class~="_attachment_file_upload"]`).prop("disabled", true);
    $.ajax({
        ...
        success: function(response) {
            $(`input[class~="_attachment_file_upload"]`).val("");
            if (response.code == 200) {
        ...
        complete: function() {
            $(`input[class~="_attachment_file_upload"]`).prop("disabled", false);
            $(`div[class~="upload-document-loader"]`).addClass("hidden");
        },
        error: function() {
            $(`input[class~="_attachment_file_upload"]`).val("").prop("disabled", false);
            $(`div[class~="upload-document-loader"]`).addClass("hidden");
        }
    });
}