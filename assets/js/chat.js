...
var format_chat_message = (chat) => {
    let position = (chat.sender_id == $myPrefs.userId) ? "chat-right" : "chat-left",
        seen_notice = ((chat.sender_id == $myPrefs.userId) && (chat.seen_time !== null)) ? `
            <span data-msg_status="seen" data-msg_id="${chat.item_id}" title="Seen at: ${chat.seen_timer}" class="text-success" style="font-size:12px;float:right;left:0px">
                <i class="fa fa-check-circle"></i></span>` : `<span data-msg_status="seen" data-msg_id="${chat.item_id}"></span>`;
    return `<div data-msg_id="${chat.item_id}" class="chat-item ${position}" style="display:none">
