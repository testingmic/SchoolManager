...
var show_change_Username_Password_form = (user_id, username, fullname, isSupport) => {
    $(`div[id="change_Username_Password"] *`).val(``);
    ...
    if(isSupport) {
        ...
        if(username !== 'no_username') {
            $(`div[id="change_Username_Password"] input[name="username"]`).attr("disabled", true);
        } else {
            $(`div[id="change_Username_Password"] input[name="username"]`).val('');
        }
    }
}
...
var search_By_Fullname_Unique_ID = () => {
    ...
    if (!lookup.length) {
        ...
        $.get(`${baseUrl}api/users/quick_search`, { lookup: lookup }).then((response) => {
            if (response.code === 200) {
                let results_list = ``,
                    isSupport = response.data.additional.isSupport,
                        ...
                        <div class="col-md-12">
                            <table width="95%" border="0">
                                <tr>
                                    ...
                                    <td>
                                        <span onclick="return show_change_Username_Password_form('${data.user_id}','${data.username == null ? "no_username" : data.username}','${data.name}', ${isSupport})" class="underline text-danger" title="Click to view edit user login details">
                                ...
                                <tr>
                                    ...
                                    <td><a href="${baseUrl}schools/${data.client_id}">${data.client_name.toUpperCase()}</a></td>
                                </tr>` : ""}
                                <tr>
...
var change_Username_Password = () => {
    ...
    let password = $(`div[id="change_Username_Password"] input[name="passwd"]`).val(),
        password_2 = $(`div[id="change_Username_Password"] input[name="passwd_2"]`).val();

    if(password.length < 8) {
        notify("Sorry! The password should be at least 8 characters long.");
        return false;
    }

    if(password !== password_2) {
        notify("Sorry! The passwords do not match.");
        return false;
    }
